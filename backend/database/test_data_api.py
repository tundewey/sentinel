...
def get_cluster_identifier():
    """Get Aurora cluster identifier used for fallback discovery."""
    return os.getenv("AURORA_CLUSTER_IDENTIFIER", "sentinel-aurora-cluster")
...
def get_cluster_details(region):
        ...
        try:
            ...
            if response['DBClusters']:
                ...
                if not cluster.get('HttpEndpointEnabled', False):
                    ...
                    print(
                        f"💡 Run: aws rds modify-db-cluster --db-cluster-identifier {get_cluster_identifier()} --enable-http-endpoint --apply-immediately"
                    )
                    return None, None
            else:
    ...
    try:
        # Get cluster ARN
        response = rds_client.describe_db_clusters(
            DBClusterIdentifier=get_cluster_identifier()
        )
        
        if not response['DBClusters']:
            print(f"❌ Aurora cluster '{get_cluster_identifier()}' not found")
            return None, None
...
def test_data_api(cluster_arn, secret_arn, region, database_name):
    ...
    try:
        ...
        if error_code == 'BadRequestException':
            ...
            print(f"   ⚠️  Database '{database_name}' might not exist or credentials are incorrect")
            ...
            try:
                ...
                print(f"   ✅ Connection successful (but '{database_name}' database may not exist)")
                return True
            except:
                pass
        else:
    ...
    try:
        response = client.execute_statement(
            ...
            secretArn=secret_arn,
            database=database_name,
            sql=f"SELECT pg_database_size('{database_name}') as size_bytes"
        )
...
def main():
    ...
    # Test the Data API
    success = test_data_api(cluster_arn, secret_arn, region, database_name)
